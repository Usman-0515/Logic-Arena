...
//functions 
void menu();                                              //to show main menu
int subject();                                            //take subjects
int difficulty();                                         //take diffculty level
int loadQuestions(string &filename, string ques[],        //Take questions from files
	string A[], string B[], string C[], string D[],
	char ans[], int MAX); 
void displayQuestion(int total);                          //shufles and show one by one questions                        
void saveScore(string playername, int finalscore);        //scores calculations
void Leaderboard();                                       //top five players
void reviewWrong(int totalAsked);                         //Shows wrong questions with correct answers
void showlifelines(int &index, int total, int used[]);    //Lifeline menu 
void applyswap(int &index, int total, int used[]);        //In lifeline to swap
void adminmode(); 
void saveQuizLog(string playername, int totalAsked, int finalScore);
...
void displayQuestion(int total)
{
	...
	saveQuizLog(playername, count, score); //quiz logs 
...
{
	ofstream fout;
	fout.open("quizlogs.txt", ios::app);   // append mode

	
	fout << playername << " ";

	// Subject & difficulty as numbers
	fout << sub << " " << diff << " ";
	fout << finalScore << " ";

	// Save asked question indexes
	for (int i = 0; i < totalAsked; i++)
		fout << askedIndex[i] << " ";
